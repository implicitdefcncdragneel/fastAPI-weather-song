from fastapi import APIRouter,status #https://fastapi.tiangolo.com/reference/apirouter/
...
async def song_recommendation(city_name: str,current_mood: str):
    ...
    Returns:
        List[str]: An array containing names of songs suggested based on the user's mood and the current weather.
    """
    latitude, longitude = await get_geo_location(city_name)
    weather_description = await get_current_weather(latitude, longitude)
    result = weather_description if check_mood_weather_match(current_mood, weather_description) else current_mood
